# Creates the simulation files for running the sensitivity tests (Appendix B) on a cluster.
...
for(i in c("dR_dH","dR_dP","dH_dP")){
  for(j in 1:n_blocks){
    sim_file_name = paste(i,j, "code.R",sep="_")
    sim_file_name = paste("Code/simulating_dynamics_in_R/server_code/",sim_file_name,sep="/")
    script_file_name = paste(i,j, "script.sh",sep="_")
    script_file_name = paste("Code/scripts/",script_file_name,sep="/")
    sink(sim_file_name)
...
starting_values = as.matrix(starting_values)
'
      )
    cat('current_data = read.csv("data/', i, 'data.csv")',sep=c('','_','_'))
    cat('\n')
    cat('n_dat = nrow(starting_values)
')
...
model_outputs = parApply(cl=cl,X=starting_values,MARGIN=1,FUN= InitiateSims, 
                         parameter_values =current_data, conversion_costs = T)
...
#This section writes scripts to run the R files. It's configured to run on the collose server. 
#This will need to be tweeked to run on a different server / username
