#require(deSolve)
#Functions for the Tritrophsim project to run and test code
...
RunSims = function(starting_points, parameter_values, run_length=10000,
                   ...
                   conversion_costs=F, 
                   testing_function = TestModelTypes, use_jacob=F, rescale_start=T){
  ...
  for(i in 1:nrow(parameter_values)){
    ...
    if(rescale_start){
      current_starting_point = starting_points*parameter_values$K[i]
    }else{
    ...
    while(restart_sample){
      ...
      if(is.na(model_output[1,1])){
        model_summary = data.frame(is_cyclic=NA,is_asym = NA, is_async=NA,
                                   ...
                                   const_cyclic=NA, const_asym=NA, const_async=NA)
      }else{
    ...
    output_data[i,names(output_data)[names(output_data)%in%names(model_summary)]] = model_summary 
